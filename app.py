...
# IMPORTANT: must be the first Streamlit command (after importing streamlit).
st.set_page_config(
...
def scenario_multipliers(scenario: str) -> dict:
    ...
    return {"rev_mult": 1.00, "churn_mult": 1.00, "eff_mult": 1.00}  # Moderado
...
def calc_coi_nrr(inp: dict) -> dict:
    ...
    # Impacts (replicando a lógica observada na aba COI NRR)
...
def render_calc_1(excel_bytes: bytes) -> None:
    ...
    with col_help:
        st.info(
            "As premissas iniciais são carregadas do Excel (abas COI Retention / COI NRR) e viram inputs interativos. "
            "Os outputs são recalculados em tempo real e salvos no session_state para a Calculadora 2."
        )
    ...
    # Defaults vindos do Excel (exemplos reais do arquivo anexado)
    # Retention
    ...
    with left:
        ...
        with c2:
            ...
            cost_per_hour = st.number_input("Cost per hour per agent", min_value=0.0, max_value=1_000.0, value=float(d_cost_per_hour_nrr if coi_mode == "NRR" else d_cost_per_hour), step=1.0)

        with c3:
            customers = st.number_input("Customers", min_value=0.0, max_value=1e9, value=float(d_customers), step=10_000.0, format="%.0f")
            baseline_churn_rate = st.slider("Baseline churn rate", min_value=0.0, max_value=0.50, value=float(d_base_churn_nrr if coi_mode == "NRR" else d_base_churn), step=0.001, format="%.3f")
        ...
        with st.expander("Parâmetros avançados", expanded=False):
            ...
            st.caption("No modo NRR, estes parâmetros extras controlam o risco (Retry/FCR/CSAT/CES/Upsell).")
            ...
            with st.expander("Inputs NRR", expanded=True):
                ...
                with n1:
                    ...
                    churn_risk_multiplier = st.number_input(
                        "Churn risk multiplier (aplica em Retry Rate)",
                        min_value=0.0,
                        max_value=50.0,
                ...
                with n3:
                    ...
                    ces_divisor = st.number_input("CES divisor (para converter p/ 0-1)", min_value=1.0, max_value=20.0, value=7.5, step=0.5)
...
def build_summary_markdown(calc1_out: dict, bi_out: dict) -> str:
    ...
    lines = []
    lines.append(f"# Resumo - COI & Business Impact")
    lines.append(f"- Gerado em: {ts}")
    lines.append("")
    lines.append("## Calculadora 1 (COI)")
    lines.append(f"- Mode: {calc1_out.get('mode')}")
    lines.append(f"- Labor inefficiency: {fmt_currency(calc1_out.get('labor_inefficiency_cost', 0.0))}")
    lines.append(f"- Revenue at risk: {fmt_currency(calc1_out.get('revenue_at_risk', 0.0))}")
    ...
    lines.append(f"- Total Cost of Inaction: {fmt_currency(calc1_out.get('total_cost_of_inaction', 0.0))}")
    lines.append("")
    lines.append("## Calculadora 2 (Business Impact)")
    lines.append(f"- Cenário: {bi_out.get('scenario')}")
    lines.append(f"- Gross benefit: {fmt_currency(bi_out.get('gross_benefit', 0.0))}")
    lines.append(f"- Annual solution cost: {fmt_currency(bi_out.get('annual_solution_cost', 0.0))}")
    lines.append(f"- Net impact: {fmt_currency(bi_out.get('net_impact', 0.0))}")
    pb = bi_out.get("payback_months", np.inf)
    lines.append(f"- Payback (meses): {('∞' if np.isinf(pb) else f'{pb:.1f}')}")
    roi = bi_out.get("roi", np.nan)
    lines.append(f"- ROI: {('n/a' if np.isnan(roi) else f'{roi:.2f}x')}")
    lines.append("")
    return "\n".join(lines)
...
def render_calc_2() -> None:
    ...
    with top_l:
        ...
        with i1:
            churn_reduction_pct = st.slider(
                "Churn reduction sobre Revenue at risk",
                0.0,
                1.0,
                value=0.25,
                step=0.01,
                help="Quanto do 'Revenue at risk' você acredita recuperar.",
            )
            revenue_impact_pct = st.slider(
                "Revenue impact sobre Upsell shortfall (NRR)",
                0.0,
                1.0,
                value=0.20,
                step=0.01,
                help="Só terá efeito se a Calc 1 estiver em modo NRR (com Upsell risk).",
            )
        with i2:
            labor_recovery_pct = st.slider(
                "Labor recovery sobre inefficiency",
                0.0,
                1.0,
                value=0.30,
                step=0.01,
                help="Quanto do custo de ineficiência de horas você reduz.",
            )
            annual_solution_cost = st.number_input(
                "Custo anual da solução (OPEX/Contrato)",
                min_value=0.0,
                max_value=1e9,
                value=5_000_000.0,
                step=50_000.0,
    ...
    with cB:
        ...
        rows = []
        for s in scenarios:
            o = calculate_business_impact(calc1_out, s, bi_inp)
            rows.append({"Scenario": s, "Net impact": o["net_impact"], "Gross benefit": o["gross_benefit"]})
        ...
        fig = px.bar(
            ...
            y="Net impact",
            text_auto=".2s",
            title="Net impact por cenário (mantendo os mesmos inputs)",
        )
        fig.update_layout(height=360, margin=dict(l=10, r=10, t=60, b=10))
        st.plotly_chart(fig, use_container_width=True)

    # Monthly cumulative net benefit (simple)
    months = np.arange(1, 13)
    monthly_benefit = bi_out["gross_benefit"] / 12.0
    monthly_cost = bi_out["annual_solution_cost"] / 12.0
    cum_net = np.cumsum(np.repeat(monthly_benefit - monthly_cost, 12))
    ...
    df_calc1_out = pd.DataFrame([calc1_out])
    df_calc2_out = pd.DataFrame([bi_out])
    ...
    excel_bytes = df_to_excel_bytes(
        {
            "calc1_outputs": df_calc1_out,
            "calc1_components": calc1_components_df,
            "calc2_outputs": df_calc2_out,
    ...
    with e1:
        st.download_button(
            ...
            data=csv_bytes,
            file_name="results_consolidated.csv",
            mime="text/csv",
        )
    with e2:
        st.download_button(
